...
import com.tendcloud.adt.framework.base.AdtFrameworkBase;
 ...
 *
 */
public abstract class AbstractBasePage extends AdtFrameworkBase {
	...
	protected WebElement getElement(String locator) throws Exception{
		...
		if(null != list && list.size() > 0 ){
			logger.info("找到元素 ' " + list.get(0).toString() + " '");
			return list.get(0);
		}
		return null;
	}
	...
	protected List<WebElement> waitUntilElements(WebElement container, By by, int timeoutMilliseconds) throws Exception{
		...
		while (! found && ! perfTimer.hasExpired()) {//found=false; while(!found) => while(true)死循环
			try {
				...
				PerformanceTimer.wait(300);
				logger.info(ReportUtils.formatData("尚未找到元素，等待 300ms 后重新尝试查找..."));
			}
		}
		if (perfTimer.hasExpired()) {//等待元素超时失败；
			logger.error("timeout waiting for element: " + by.toString());
	...
	public void scrollIntoView(WebElement element){
		report(ReportUtils.formatAction(String.format("Scroll element %s Into Views ",element.getText())));
...
//		}
	 ...
	 * 在任意页面返回“产品中心”页面：
	 */
	public void back2ApplicationCenterPage() throws Exception{
		String locator = BasicElementLocators.TO_PRODUCT_CENTER;
